...
class NeoLinkViewModel {
    ...
    fun updatePpv2(enabled: Boolean) {
        val previous = isPpv2Enabled
        featureState = featureState.copy(ppv2Enabled = enabled)

        if (!isRunning) {
            FeatureState.setEnableProxyProtocol(enabled)
            appendSystemLog(buildPpv2UpdateMessage(enabled))
            return
        }

        scope.launch(Dispatchers.IO) {
            try {
                NeoLinkCoreRunner.updateRuntimePpv2(enabled)
                appendSystemLog(buildPpv2UpdateMessage(enabled))
            } catch (e: RuntimeException) {
                withContext(Dispatchers.Main) {
                    featureState = featureState.copy(ppv2Enabled = previous)
                    FeatureState.setEnableProxyProtocol(previous)
                    appendSystemLog("运行时 PPv2 更新失败：${e.message ?: e.javaClass.simpleName}")
                }
            }
        }
    }
