...
public class LanguageData implements Serializable {
    ...
    // 重连消息模板
    public String RECONNECT_MSG_TEMPLATE_EN = "Reconnection will begin after %d seconds.";
    public String RECONNECT_MSG_TEMPLATE_ZH = "%d 秒后将开始重新连接。";
    ...
    public static LanguageData getChineseLanguage() {
        ...
        languageData.RECONNECT_MSG_TEMPLATE_EN = "Reconnection will begin after %d seconds.";
        languageData.RECONNECT_MSG_TEMPLATE_ZH = "%d 秒后将开始重新连接。";
     ...
     * 生成重连提示文本。
     * <p>
     * 原始实现直接调用 ClientConsole.say()，此处解耦为纯文本返回，
     * 由调用方决定如何输出（CLI / GUI / Android）。
     */
    public String getReconnectMessage(int seconds) {
        if ("zh".equals(currentLanguage)) {
            return String.format(RECONNECT_MSG_TEMPLATE_ZH, seconds);
        }
        return String.format(RECONNECT_MSG_TEMPLATE_EN, seconds);
    }
